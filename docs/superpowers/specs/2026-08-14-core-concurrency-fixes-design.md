...
- 事件在步骤 6 **之后**到达 → 事件自身的 `clientSideCache.evict` 清掉新写入的**本地**值；但分布式层存在残余窗口（事件晚于写后校验时，onEvicted 不会清分布式层，旧值在该层存活至 TTL）。该窗口属本专项已知的遗留缺口，是后续"L1 条目版本化比对"机制的动机案例。

本地层三个区间全覆盖；分布式层残余窗口如上，无需 `onEvicted` 与回源持锁互斥。
